export type CmsFieldType = 'text' | 'textarea' | 'number' | 'url' | 'boolean';
...
export function buildCmsBlockHtml(type: CmsContentType, entries: CmsEntry[]): string {
  ...
  const imageField = type.fields.find((f) => f.type === 'url' && f.name.toLowerCase().includes('image'))?.name;
  ...
  <div data-cms-entry="${e.id}" style="padding:16px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:12px;background:#fff;">
    ${img ? `<img src="${img}" alt="${title}" style="width:100%;height:160px;object-fit:cover;border-radius:4px;margin-bottom:10px;" />` : ''}
  ...
  return `<section data-cms-type="${type.slug}" style="padding:24px;background:#f8fafc;">
